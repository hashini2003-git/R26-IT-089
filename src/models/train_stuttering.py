"""
train_stuttering.py — Binary classifier: stuttering vs fluent speech.

Labels: label_stuttering  (1 = stuttering, 0 = fluent)
Datasets: fluencybank (stuttering=1), mdvr_kcl / italian_parkinsons (fluent proxy=0)
"""
...
def main():
    ...
    parser.add_argument("--log",       default="logs/train_stuttering.log")
    ...
    X_train, y_train = load_split(splits, "train", feat_df, "label_stuttering")
    X_val,   y_val   = load_split(splits, "val",   feat_df, "label_stuttering")
    logger.info(f"Train: {X_train.shape}  stutter={y_train.sum()}  fluent={(y_train==0).sum()}")
    ...
    logger.info("Training stuttering classifier …")
    pipe.fit(X_train, y_train)

    y_pred = pipe.predict(X_val)
    y_prob = pipe.predict_proba(X_val)[:, 1]
    auc    = roc_auc_score(y_val, y_prob) if len(np.unique(y_val)) > 1 else float("nan")
    logger.info(f"\n{classification_report(y_val, y_pred, target_names=['fluent','stutter'])}")
    logger.info(f"Val AUC: {auc:.4f}")

    model_path = out_dir / "stuttering_model.pkl"
